...
bool g_display_effective = false;
string g_email;
...
public:
	...
	void interpretOption(int argc, char** argv)
	{
		...
		poolDesc
		        ...
		        << "ethproxy - dwarfpool, f2pool, nanopool (required for hashrate reporting to work with nanopool)\n"
		...
		("effective", bool_switch()->default_value(false), "Log effective hash rate.\n")
		...
		if (vm.find("cu-devs") != vm.end()) {
			m_cudaDeviceCount = vm["cu-devs"].as<vector<unsigned>>().size();
			m_cudaDevices = vm["cu-devs"].as<vector<unsigned>>();
		}
		...
		g_display_effective = vm["effective"].as<bool>();
